...
from drvi.utils.metrics._pairwise import nn_alignment_score, local_mutual_info_score, spearman_correlataion_score
...
class DiscreteDisentanglementBenchmark:
    ...
    def __init__(self, embed, discrete_target=None, one_hot_target=None, dim_titles=None, metrics=['SMI', 'SPN', 'ASC'], aggregation_methods=['LMS', 'MSAS', 'MSGS']):
        ...
        if discrete_target is not None:
            ...
            one_hot_target = pd.DataFrame(np.eye(len(discrete_target.cat.categories))[discrete_target.cat.codes], columns=discrete_target.cat.categories)

        if isinstance(one_hot_target, pd.DataFrame):
            pass
        elif isinstance(one_hot_target, np.ndarray):
            one_hot_target = pd.DataFrame(one_hot_target, columns=[f"process_{i}" for i in range(one_hot_target.shape[1])])
        else:
    ...
    def _compute_metrics(embed, one_hot_target, dim_titles=None, metrics=tuple()):
        ...
        for metric_name in metrics:
            result_df = pd.DataFrame(AVAILABLE_METRICS[metric_name](embed, gt_one_hot=one_hot_target.values), 
                                     index=dim_titles, 
    ...
    def _aggregate_metrics(results, aggregation_methods=tuple()):
        ...
        for aggregation_method in aggregation_methods:
            for metric_name in results:
                aggregated_results[f"{aggregation_method}-{metric_name}"] = AVAILABLE_AGGREGATION_METHODS[aggregation_method](results[metric_name].values)
