...
def make_generative_samples_to_inspect(
    ...
    if n_cats_per_key is not None:
        random_cats = torch.from_numpy(
            np.stack([np.random.randint(0, n_cat, size=n_samples) for n_cat in n_cats_per_key], axis=1)
        )
        ...
        for slice in torch.split(torch.arange(z.shape[0]), batch_size):
            ...
            gen_input = model.module._get_generative_input(
                tensors={
                    ...
                    if model.summary_stats.get("n_extra_continuous_covs", 0) == 1
                    else None,
...
def make_effect_adata(control_mean_param, effect_mean_param, var_info_df, span_limit, add_to_counts=1.0):
    ...
    add_eps_in_count_space = lambda x: torch.logsumexp(
        torch.stack([x, math.log(add_to_counts) * torch.ones_like(x)]), dim=0
    )
    ...
    effect_adata.obs["value"] = np.concatenate(
        [
            np.linspace(span_limit[0], span_limit[0] * 0.0, num=int(n_steps / 2)),
            np.linspace(span_limit[1] * 0.0, span_limit[1], num=int(n_steps / 2)),
        ],
        axis=0,
    ).T.reshape(-1)
...
def mark_differential_vars(effect_adata, layer=None, key_added="affected_vars", min_lfc=1.0):
    ...
    change_array = np.expand_dims(
        effect_adata.obs["value"].values.reshape(effect_adata.uns["n_latent"], effect_adata.uns["n_steps"]), axis=-1
    )  # n_latent x n_steps x 1
    ...
    effect_array = effect_array.reshape(
        effect_adata.uns["n_latent"], effect_adata.uns["n_steps"], effect_adata.uns["n_vars"]
    )  # n_latent x n_steps x n_vars
    for change_sign in ["-", "+"]:
        sig_genes = {}
        for effect_sign in ["up", "down"]:
            max_effect = np.max(
                effect_array * (1.0 if effect_sign == "up" else -1.0) * (change_sign_array == float(f"{change_sign}1")),
                axis=1,
            )  # n_latent x n_vars
            ...
            sig_genes[effect_sign] = [
                list(
                    zip(
                        effect_adata.var.iloc[max_indices[i, :n]].index, max_effect[i, max_indices[i, :n]], strict=False
                    )
                )
...
def combine_differential_vars(effect_adata, function, *keys):
    ...
    for dim in effect_adata.uns[any_key]:
        ...
        for change_sign in ["up", "down"]:
            result[dim][change_sign] = function(*[effect_adata.uns[key][dim][change_sign] for key in keys])
    return result


def find_differential_vars(effect_adata, method="log1p", added_layer="effect", add_to_counts=1.0, relax_max_by=1.0):
    assert method in ["log1p", "relative", "min_possible"]
    original_effect_adata = effect_adata
    original_effect_adata.uns["affected_vars"] = {}
    effect_adata = effect_adata[effect_adata.obs.sort_values(["original_order"]).index].copy()
    effect_adata = effect_adata[:, effect_adata.var.sort_values(["original_order"]).index].copy()
    ...
    find_relative_effect = lambda x, max_possible: (
        torch.logsumexp(torch.stack([x, math.log(add_to_counts) * torch.ones_like(x), max_possible]), dim=0)
        - max_possible
    )
    ...
    if method == "log1p":
        ...
        max_possible_other_dims = (
            ...
            .unsqueeze(0)
            .expand(n_latent, -1, n_steps, n_random_samples, -1)
    ...
    original_effect_adata.layers[added_layer] = effect_adata[
        original_effect_adata.obs.index, original_effect_adata.var.index
    ].layers[added_layer]


def sort_and_filter_effect_adata(effect_adata, optimal_dim_ordering, min_lfc=2.0):
    dim_order_mapping = {f"Dim {1 + o}": i for i, o in enumerate(optimal_dim_ordering)}
    effect_adata.obs["optimal_order"] = (
        effect_adata.obs["dim"].apply(lambda x: dim_order_mapping[x]).astype(int).astype("category")
    )
    ...
    effect_adata.obs["dim_id_plus"] = np.where(
        effect_adata.obs["value"] > 0,
    ...
    effect_adata.var["max_effect_order"] = np.where(
        effect_adata.var["max_effect"] > min_lfc, effect_adata.var["max_effect_change"] // n_steps, -1
    )
    ...
    effect_adata = effect_adata[
        :,
        effect_adata.var.sort_values(
            ["max_effect_order", "max_effect_dim_plus", "max_effect_sign", "max_effect"],
...
def iterate_and_make_effect_adata(
    model, adata, n_samples=100, noise_stds=0.5, span_limit=3, n_steps=10 * 2 + 1, min_lfc=1.0
):
