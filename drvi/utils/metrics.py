...
def nn_alignment_score_per_dim_per_binary_ct(var_continues, ct_binary):
    order = var_continues.argsort()
    ct_binary = ct_binary[order]
    alignment = np.clip(
        (np.sum((ct_binary[:-1] == 1) & (ct_binary[1:] == 1)) / (np.sum(ct_binary == 1) - 1))  # fraction of cells of this type that are next to a cell of the same type
        -
        (np.sum(ct_binary == 1) / ct_binary.shape[0]),  # cancel random neighbors when CT is frequent
        0, None,
    )
    return alignment
